...
int openingWindow(int argc, char *argv[])
{
    GtkBuilder * builder ;
    GtkWidget   * connectButton  , * inputIPaddr , * inputPassword   , * inputIPaddrMsg , * inputPasswordMsg  ;
    ...
    if(isPasswordCorrect){
        std::cout << "true :: password is correct quited mainloop" << std::endl;
    }
    return (int)isPasswordCorrect;
}
...
void on_btn_connect_clicked(GtkWidget *widget,gpointer   data){
    ...
    if(std::string(ipp.ip) == "127.0.0.1"){
         printf("used same ip address \n");
        isPasswordCorrect = true;
    }
